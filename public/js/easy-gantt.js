$.fn.gantt = function (options) {
    ...
    for(let i = 0; i <= countDays-1; i++){
        let day = moment(firstDay, "DD/MM/YYYY").add(i + 1, "days").format('DD');
        let dayNumber = moment(firstDay, "DD/MM/YYYY").add(i + 1, "days").dayOfYear();
        ...
        if(tooltipShow){
            $('#tbody_'+unic+' > tr > .td-tasks').on('mouseover', function(){ // Cria o tooltip ao passar o mouse na atividade
                let tooltipGantt = `<div class="tooltip-gantt">
                                    ...
                                    <span>${$(this).attr('task_days')} Day(s)</span>
                                    <hr>
